...
void storemsg(){
	int wfile = open(D_WFILE, O_CREAT | O_WRONLY | O_APPEND);
...
void readmsg(){
	int rfile = open(D_RFILE, O_CREAT | O_RDONLY);
...
int main(){
	//while(1){
	    ...
	    int filedesc = open("tmp/dane.txt", O_CREAT | O_RDONLY);
		if (filedesc < 0) {
	    ...
	    int filedesc2 = open("tmp/wyniki.txt", O_CREAT | O_WRONLY | O_APPEND);
		if (filedesc2 < 0) {
